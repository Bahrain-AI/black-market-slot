...
function openBuy(){if(spinning||bonusActive)return;stopAutoplay();selectedBuy=null;buyConfirm.hidden=true;bonusCards.forEach(function(c){c.classList.remove('selected');});bonusModal.classList.add('open');bonusModal.setAttribute('aria-hidden','false');el('stage').inert=true;bonusClose.focus();}
function closeBuy(){if(bonusActive)return;bonusModal.classList.remove('open');bonusModal.setAttribute('aria-hidden','true');el('stage').inert=false;selectedBuy=null;buyBtn.focus();}
function renderBuy(key){var cfg=BUY_MODES[key];if(!cfg)return;selectedBuy=key;bonusCards.forEach(function(c){c.classList.toggle('selected',c.getAttribute('data-mode')===key);});confirmName.textContent=cfg.name;confirmCost.textContent=money(BET*cfg.costMult);confirmBuy.disabled=BALANCE<BET*cfg.costMult;confirmBuy.textContent=confirmBuy.disabled?'INSUFFICIENT BALANCE':'CONFIRM PURCHASE';buyConfirm.hidden=false;buyConfirm.scrollIntoView({block:'nearest'});}
...
function startBonus(key){if(spinning||bonusActive)return;var cfg=BUY_MODES[key];if(!cfg)return;var cost=BET*cfg.costMult;if(BALANCE<cost){showMessage('INSUFFICIENT BALANCE',1500);renderBuy(key);return;}BALANCE-=cost;closeBuy();turboBtn.focus();bonusActive=true;bonusEnding=false;bonusMode=cfg;bonusSpins=cfg.spins;bonusMultiplier=cfg.startMult;bonusTotal=0;bonusSpecialRate=cfg.specialRate;bonusMods=chooseMods(cfg);lockControls(true);stopAutoplay();updateHUD();var mods=bonusMods.length?' · '+bonusMods.map(modLabel).join(' + '):'';splash('ACCESS GRANTED · '+cfg.name+mods,1200);setTimeout(spin,turbo?500:1250);}
...
document.addEventListener('keydown',function(e){
  if(e.code==='Tab'&&bonusModal.classList.contains('open')){
    var buttons=[].slice.call(bonusModal.querySelectorAll('button:not(:disabled)')).filter(function(b){return b.getClientRects().length;});
    var first=buttons[0],last=buttons[buttons.length-1];
    if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}
    else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}
  }
  // Native buttons own Space/Enter; the game shortcut must not steal activation.
  if(e.code==='Space'&&e.target.closest('button'))return;
  var open=modalEl.classList.contains('open')||bonusModal.classList.contains('open');if(e.code==='Escape'){if(modalEl.classList.contains('open'))closeInfo();if(bonusModal.classList.contains('open'))closeBuy();}if(e.code==='Space'&&!open&&!spinning&&!bonusActive){e.preventDefault();if(autoplay)stopAutoplay();spin();}if(!open&&!bonusActive&&(e.key==='ArrowLeft'||e.key==='-')){e.preventDefault();shiftBet(-1);}if(!open&&!bonusActive&&(e.key==='ArrowRight'||e.key==='+'||e.key==='=')){e.preventDefault();shiftBet(1);}if(!open&&!bonusActive&&(e.key==='b'||e.key==='B'))openBuy();});
